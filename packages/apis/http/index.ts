...
const transform: AxiosTransform = {
   ...
   */
  responseSuccessHook(res: AxiosResponse<Result>, options: RequestOptions) {
    const { t } = useI18n();
    ...
    if (!data) {
      ...
      throw t('sys.api.apiRequestFailed');
    }
    ...
    throw timeoutMsg || t('sys.api.apiRequestFailed');
  },
  /**
   * @description: 响应错误处理
   */
  responseCatchHook(error: AxiosError<Result, any>, options: RequestOptions) {
    ...
    const { isOpenRetry } = options.retryRequest!;
